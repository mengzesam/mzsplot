...
"""
import FileDialog  #pyinstaller require
import numpy as np
...
if __name__=='__main__':
    import sys
    if len(sys.argv)>=2:
        bookfile=sys.argv[1] #.decode('utf-8').encode('utf-8')
        wb=xw.Book(bookfile)
        sht=wb.sheets[2]
        tagNum=int(sht.range('A4').value)
        rowNum=int(sht.range('A10').value)    
        if tagNum>0 and rowNum>0:
            if tagNum==1:
                Tags=np.array([sht.range('B2:B%d'%(2+tagNum-1)).value])
                ysMin=np.array([sht.range('C2:C%d'%(2+tagNum-1)).value])
                ysMax=np.array([sht.range('D2:D%d'%(2+tagNum-1)).value])
                ysColor=np.array([sht.range('E2:E%d'%(2+tagNum-1)).value])
                ysPhiFlag=np.array([sht.range('F2:F%d'%(2+tagNum-1)).value])        
            else:
                Tags=np.array(sht.range('B2:B%d'%(2+tagNum-1)).value)
                ysMin=np.array(sht.range('C2:C%d'%(2+tagNum-1)).value)
                ysMax=np.array(sht.range('D2:D%d'%(2+tagNum-1)).value)
                ysColor=np.array(sht.range('E2:E%d'%(2+tagNum-1)).value)
                ysPhiFlag=np.array(sht.range('F2:F%d'%(2+tagNum-1)).value)
            drawStyle=int(sht.range('G2').value)
            winTitle=sht.range('A2').value
            x=np.array(sht.range('H2:H%d'%(2+rowNum-1)).value)
            ys=np.array(sht.range((2,9),(1+rowNum,8+tagNum)).value)
            if ys.ndim==1:
                ys.size
                ys=ys.reshape(ys.size,1)                
            app = QtGui.QApplication(sys.argv)
            mzsplot_Trend(winTitle, x, ys ,Tags, ysMin,ysMax, ysColor,ysPhiFlag,drawStyle)
            sys.exit(app.exec_())
